...
namespace AiChef.Server.Services
{
    public class OpenAIService : IOpenAIAPI
    {
        ...
        private static ChatFunction.Parameter _recipeParamter = new()
        {
            Type = "object",
        ...
        public async Task<List<Idea>> CreateRecipeIdeas(string mealTime, List<string> ingredients)
        {
            ...
            string ingredientItems = string.Join(",",ingredients);

            if (string.IsNullOrEmpty(ingredientItems))
            {
                ingredientPrompt = "Suggest some ingredients for me";
            }
            else
            {
                ingredientPrompt = $"I have {ingredientItems}";
            }
